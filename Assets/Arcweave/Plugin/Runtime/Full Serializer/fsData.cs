...
namespace Arcweave.FullSerializer
{
    ...
    public sealed class fsData
    {
        ...
        public fsData this[string key] {
            get
            {
                var i = key.IndexOf('.');
                var thisKey = i == -1 ? key : key.Substring(0, i);
                if ( IsDictionary && AsDictionary.TryGetValue(thisKey, out var result) ) {
                    return i == -1 ? result : result[key.Replace(thisKey + '.', string.Empty)];
                }
                return null;
            }
        }
        ...
        public static fsData CreateDictionary() {
            return new fsData(new Dictionary<string, fsData>(StringComparer.OrdinalIgnoreCase));
        }
